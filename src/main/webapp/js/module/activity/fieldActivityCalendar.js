...
function createCalendar(a) {
    ...
    caHtml.append("<th><img src='" + ctx + "/images/calendar/2.jpg' /></th>");
    caHtml.append("<th><img src='" + ctx + "/images/calendar/3.jpg' /></th>");
    caHtml.append("<th><img src='" + ctx + "/images/calendar/4.jpg' /></th>");
    caHtml.append("<th><img src='" + ctx + "/images/calendar/5.jpg' /></th>");
    caHtml.append("<th><img src='" + ctx + "/images/calendar/6.jpg' /></th>");
    caHtml.append("<th><img src='" + ctx + "/images/calendar/7.jpg' /></th>");
    caHtml.append("<th class='last'><img src='" + ctx + "/images/calendar/8.jpg' /></th>");
    caHtml.append("</tr></thead>"); /*第一行*/
    caHtml.append("<tr>");

    var date = a.getDate();
	if ($.common.browser.isMozila() || $.common.browser.isChrome()) {
		a.setDate(3);
	} else {
		a.setDate(1);
	}
    ...
    // FF浏览器年加上1900
    if (year < 1000 && ($.common.browser.isMozila() || $.common.browser.isChrome())) {
        year += 1900;
    }
...
function getDateStatus(dealedDate) {
    ...
    function callJsonp() {
    	$.jsonp( {
			...
			data : {
				action : 'fieldActivityStatus',
				fieldType : $('#fieldType').val(),
				venueId : cvi,
				fromDate : dealedDate
			},
			success : function(status) {
				_getDateStatus(status);
			},
    ...
    function _getDateStatus(status) {
        var tempDate = null;
        ...
        if (pageDate == 1) {
            dealedDateObj = $.common.custom.dateAdd(dealedDate, -1, 'd');
        }
        $('.mainBody ol').html('');
        ...
        //log.debug(tempSysteDate.toDateStr());
        for (var i = 1; i <= 7; i++) {
...
function enterSelectPriod() {
    ...
    $.nyroModalManual({
        ...
        height: 550,
        width: 800,
        url: 'loadActivityGrid.action?usableDate=' + bookingDate + '&fieldType=' + fieldType
    });
