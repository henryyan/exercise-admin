...
function deletePrice() {
	...
	$.get(ctx + '/price/' + fieldType + '-' + priceType + '!delete.action', {
		id: data.pid
	}, function(rep) {
		trDom.remove();
	});
...
function savePrice() {
	...
	$.post(ctx + '/price/' + fieldType + '-' + priceType + '!updatePrice.action', {
		id: data.pid,
		price: price,
		priceType: pt
	}, function(req){
...
function next() {
	...
	$('.configure').fadeOut('fast', function(){
		...
		var tempStartHour = parseInt(startTime.split(':')[0]);
		var tempEndHour = parseInt(endTime.split(':')[0]);
		var totalHours = tempEndHour - tempStartHour;

		var ct = totalHours / timeStep;
		var tempPriceWorkspace = "";
		var tempTime = addTime(startTime, 0);
		for(var i = 0; i < ct; i++) {
			...
			var tmpHour = parseInt(tempTime.split(':')[0]);
			...
			tempPriceWorkspace += "<td width='90' style='text-align:center'>" + singleTime 
				+ "</td><td class='period'><input type='text' id='price' name='price' size='6' maxlength='3'/></td>" 
				+ "<td><input type='text' name='protocolPrice' size='6' maxlength='3'/></td></tr>";
			
			if (tempTime == endTime ) {break;}
		}
