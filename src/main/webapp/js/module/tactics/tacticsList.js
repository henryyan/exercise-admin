...
$(function() {
	...
	$('#editDate').bind('click', function(){
		var settings = {
            objId: $(this).parent().metadata().idv
        };
		editDate(settings);
	});
...
function setTipInfo() {
	...
	$('a[name=showDate]').cluetip({
		...
		onShow: function(ct, c){
			var id = $(this).parent().metadata().idv;
			$.getJSON('tacticsDate.action', {
				tacticsId: id
			}, function(result){
...
function registeOperation() {
    $('.operate a').click(function(event) {
    	var idv = $(this).parent().metadata().idv;
	...
	$('.operate :button').click(function(event) {
		var srcEleName = $(this).attr('name');
        var settings = {
            srcEle: this,
            objId: $(this).parent().metadata().idv,
			fieldType: $(this).parent().metadata().ft
        };
...
function modify(settings) {
	...
	$.nyroModalManual({
		...
		endShowContent: function(modal, currentSettings) {
			$.getJSON(ctx + '/tactics/tactics!findTactics.action', {
				id: settings.objId
			}, function(tactics){
...
function deleteTacticsDate() {
	...
	$.get(ctx + '/tactics/tacticsDate!delete.action', {
        id: objId
    }, function(rep) {
        if (rep == 'true') {
        	$tr.remove();
...
function deleteTactics(settings) {
	...
	var activityTotal = 0;
	var $tr = $(settings.srcEle).parents('tr');
	
	$.common.system.getExerciseUrl({
		callback : function(exerciseUrl) {
			var fieldType = $(settings.srcEle).parent().metadata().ft;
			$.jsonp( {
	...
	function innerDelete() {
		...
		$.get(ctx + '/tactics/tactics!delete.action', {
	        id: settings.objId
	    }, function(rep) {
	        if (rep == 'true') {
	        	if (dialog) {
	 ...
	 * 把和此策略相关的活动用默认的策略覆盖
	 */
	function applyDefaultTactics(settings) {
		$.blockUI({
	        message:  '<h5><img src="' + ctx + '/images/ajaxLoader.gif" class="status"/>正在应用默认策略，请稍候……</h5>',
			css: { width: '400px' }
	    });
...
function addDate(settings) {
    ...
    $('#addDate').unbind('click').click(function() {
        ...
        $.getJSON('tacticsDate!save.action', {
            tacticsId: settings.objId,
...
function deleteAndBakActivity(settings, callback) {
	$.blockUI({
        message:  '<h5><img src="' + ctx + '/images/ajaxLoader.gif" class="status"/>正在更新已生成的活动，请稍候……</h5>',
		css: { width: '400px' }
    });
