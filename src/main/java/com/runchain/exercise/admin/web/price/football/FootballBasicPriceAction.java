 ...
 */
@Namespace("/price")
@Action("football-basic")
@Results( { @Result(location = "price-basic.jsp") })
public class FootballBasicPriceAction extends ActionSupport {

	private static final long serialVersionUID = 1L;
	
	protected Logger logger = LoggerFactory.getLogger(getClass());

	@Resource(name = "footballBasicPriceManager")
	protected IPriceManager<FootballBasicPrice> manager;
	private Long id;
	private String fieldType = "football";
	private String priceType = "basic";
	private List<FootballBasicPrice> prices;
	...
	public String batchAdd() {
		try {
			...
			logger.error("批量添加[足球][非周末]价格时：", e);
	...
	public String updatePrice() {
		...
		if (StringUtils.isNotEmpty(strId) && StringUtils.isNotEmpty(strPrice)) {
			FootballBasicPrice price = manager.getPrice(new Long(strId));
	...
	public String list() {
		try {
			...
			logger.error("获取[足球][非周末]价格列表时：", e);
		}
		return SUCCESS;
	}
	...
	public List<FootballBasicPrice> getPrices() {
		return prices;
	}
