 ...
 */
@Namespace("/price")
@Action("badmintoon-weekend")
@Results( { @Result(location = "price-basic.jsp") })
public class BadmintoonWeekendPriceAction extends ActionSupport {

	private static final long serialVersionUID = 1L;
	
	protected Logger logger = LoggerFactory.getLogger(getClass());

	@Resource(name = "badmintoonWeekendPriceManager")
	protected IPriceManager<BadmintoonWeekendPrice> manager;
	private Long id;
	private String fieldType = "badmintoon";
	private String priceType = "weekend";
	private List<BadmintoonWeekendPrice> prices;
	...
	public String batchAdd() {
		try {
			...
			logger.error("批量添加[羽毛球][周末]价格时：", e);
	...
	public String updatePrice() {
		...
		if (StringUtils.isNotEmpty(strId) && StringUtils.isNotEmpty(strPrice)) {
			BadmintoonWeekendPrice price = manager.getPrice(new Long(strId));
	...
	public String list() {
		try {
			...
			logger.error("获取[羽毛球][周末]价格列表时：", e);
		}
		return SUCCESS;
	}
	...
	public List<BadmintoonWeekendPrice> getPrices() {
		return prices;
	}
