...
public class TennisBasicPriceAction extends ActionSupport {
	...
	protected IPriceManager<TennisBasicPrice> manager;
	private Long id;
	private String fieldType = "tennis";
	private String priceType = "basic";
	private List<TennisBasicPrice> prices;
	...
	public String batchAdd() {
		try {
			...
			logger.error("批量添加[网球][非周末]价格时：", e);
	...
	public String updatePrice() {
		...
		if (StringUtils.isNotEmpty(strId) && StringUtils.isNotEmpty(strPrice)) {
			TennisBasicPrice price = manager.getPrice(new Long(strId));
	...
	public String list() {
		try {
			...
			logger.error("获取[网球][非周末]价格列表时：", e);
		}
		return SUCCESS;
	}
	...
	public List<TennisBasicPrice> getPrices() {
		return prices;
	}
